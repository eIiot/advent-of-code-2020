var input = `eyr:2029 pid:157374862
...
const inputNoNewlines = input.split('\n\n');

const mappedData = [];

var totalValid = 0;

for (let i = 0; i < inputNoNewlines.length; i++) {
  inputNoNewlines[i] = inputNoNewlines[i].replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' ').replace('\n', ' '); // I don't know why this works and at this point I'm too afraid to ask

  const table =
    inputNoNewlines[i].split(" ") //["key:value" "key:value"]
  ...
  // validate fields 
  var byr = parseInt(result.get('byr'));
  var iyr = parseInt(result.get('iyr'));
  var eyr = parseInt(result.get('eyr'));
  var hgt = result.get('hgt');
  var hcl = result.get('hcl');
  var ecl = result.get('ecl');
  ...
  if (hgt !== undefined) {
    if (hgt.endsWith('cm') && parseInt(hgt.substring(0, hgt.length - 2)) >= 150 && parseInt(hgt.substring(0, hgt.length -2)) <= 193) {
      fieldsValid++;
    };
    if (hgt.endsWith('in') && parseInt(hgt.substring(0, hgt.length - 2)) >= 59 && parseInt(hgt.substring(0, hgt.length - 2)) <= 76) {
      fieldsValid++;
    };
  };
  
  if (hcl !== undefined && hcl.startsWith('#')) {
    var letters = /^[0-9a-f]+$/;
    if (letters.test(hcl.substring(1)) && hcl.length === 7) {
      fieldsValid++;
    };
  };
