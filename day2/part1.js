...
for (let i = 0; i < arraySplit.length; i++) {
  ...
  var char = element[2]
  var minChar = element[0]
  var maxChar = element[1]

  var occurances = password.replace(new RegExp(`[^${char}]`, 'g'), '').length

  console.log(password.replace(new RegExp(`[^${char}]`, 'g'), ''))

  console.log(occurances);

  element[4] = occurances;

  if (occurances >= minChar && occurances <= maxChar) {
    element[5] = true
    validPasswords++;
  } else {
    element[5] = false
