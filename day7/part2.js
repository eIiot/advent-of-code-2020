var input = `shiny plum bags contain no other bags.
...
for (let i = 0; i < rules.length; i++) {
  ...
  const rule2 = rule.split('contain').map(function(element){return element.trim()});

  rule2[0] = rule2[0].slice(0,-5);

  rule2[1] = rule2[1].split(',').map(function(element){return element.trim().replace(/[^\w\s]|_/g, "").replace(/\s+/g, " ")});

  for (let j = 0; j < rule2[1].length; j++) {
    const bag = rule2[1][j];
