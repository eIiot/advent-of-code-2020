...
// array with the entire alphabet inside
...
for (let i = 0; i < inputs.length; i++) {
  groups[i] = [];
  groups[i].people = inputs[i].split('\n');
};
...
for (let i = 0; i < groups.length; i++) {
  ...
  for (let j = 0; j < group.people.length; j++) {
    const person = group.people[j];
    ...
    for (let k = 0; k < alphabet.length; k++) {
      ...
      if (group.letters[letter] == undefined ) {
        group.letters[letter] = 0;
      };

      if (regex.test(person)) {
        group.letters[letter]++;
      };
    };
  };
...
for (let i = 0; i < groups.length; i++) {
  ...
  for (let j = 0; j < Object.keys(group.letters).length; j++) {
    const letter = group.letters[alphabet[j]];
    
    if (letter === group.people.length) {
      totalNumOfYes++;
    };
  };
};
