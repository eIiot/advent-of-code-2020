...
console.log(input.toString().replace(/[^L]/g, "").length)
...
// check every cell for rules, then move to new array
...
function checkNewRows() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < row.length; j++) {
      ...
      /// find first cell that's not a floor for each direction

      let k = 1;

      while (surrCells.includes('.') || surrCells[0] == undefined) {
        if (surrCells[0] == undefined || (surrCells[0] !== '#' && surrCells[0] !== 'L')) {
          surrCells[0] = getCell(x,y-k);
        };
        if (surrCells[1] == undefined || (surrCells[1] !== '#' && surrCells[1] !== 'L')) {
          surrCells[1] = getCell(x,y+k);
        };
        if (surrCells[2] == undefined || (surrCells[2] !== '#' && surrCells[2] !== 'L')) {
          surrCells[2] = getCell(x-k,y);
        };
        if (surrCells[3] == undefined || (surrCells[3] !== '#' && surrCells[3] !== 'L')) {
          surrCells[3] = getCell(x+k,y);
        };
        if (surrCells[4] == undefined || (surrCells[4] !== '#' && surrCells[4] !== 'L')) {
          surrCells[4] = getCell(x-k,y-k);
        };
        if (surrCells[5] == undefined || (surrCells[5] !== '#' && surrCells[5] !== 'L')) {
          surrCells[5] = getCell(x+k,y-k);
        };
        if (surrCells[6] == undefined || (surrCells[6] !== '#' && surrCells[6] !== 'L')) {
          surrCells[6] = getCell(x-k,y+k);
        };
        if (surrCells[7] == undefined || (surrCells[7] !== '#' && surrCells[7] !== 'L')) {
          surrCells[7] = getCell(x+k,y+k);
        };
      ...
      for (let k = 0; k < surrCells.length; k++) {
        const cell = surrCells[k];
      ...
      // apply rules and change seat

      var newSeat =  seat;

      if (seat == 'L' && fullCells == 0) {
        newSeat = '#';
      } else if (seat =='#' && fullCells > 4) {
        newSeat = 'L';
      };
...
while (previousRows.toString() !== rows.toString()) {
  checkNewRows();
  console.log(rows)
  console.log(countSeats(rows));
};
